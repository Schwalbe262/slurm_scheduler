...
class Scheduler:
    def __init__(
        ...
        self._task_reservation_lock = threading.RLock()
    ...
    def close_allocation(
        ...
        *,
        ...
        # Placement and shutdown share one lock.  Without it, a web fast-path
        # could reserve a task after the final claim check but before scancel.
        # Keep the lock through the remote cancellation so no new owner can be
        # attached to an allocation once shutdown has started.
        with self._task_reservation_lock:
    ...
    def reserve_task_on_allocation(self, task: dict, allocation: dict, account: AccountConfig) -> dict | None:
        with self._task_reservation_lock:
            # Another fast-path caller may have claimed this previously empty
            # CPU allocation after placement but before this transition.
    ...
    def fea_immature_footprint(self, node_name: str) -> dict[str, float]:
        ...
        CPU is reserved only for a short observation window; after that the
        fresh load/utilization sample already includes the worker and adding
        its declaration again would double-count CPU for the full RAM window.
        """
    ...
    def fea_dynamic_extra_slots(self, allocation: dict, task: dict) -> int:
        ...
        memory_slots = memory_budget // self.fea_shared_memory_estimate_mb(task)
    ...
    def task_requested_account_name(task: dict) -> str:
        """Return the user's placement constraint, not the last execution account.
