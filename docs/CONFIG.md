...
- `license_monitor`: FlexLM license polling for the dashboard's ANSYS Licenses panel and `GET /api/licenses`. Runs `<lmutil_path> lmstat -c <license_server> -a` over SSH from `account` every `interval_seconds` (default 60) on a background thread. The panel lists features currently in use plus any `watch_features`.
- `license_monitor.admission`: optional fail-closed admission for persistent task license seats. `persistent_cost_by_project` maps a project to features held for its whole run; `reserve_by_feature` leaves operator headroom. A task is held queued when the latest successful snapshot is older than `snapshot_max_age_seconds`, the server/error/feature/count evidence is invalid, its FEA project has no profile while `unknown_fea_project_policy: block`, or `used + unsettled + candidate` exceeds `total - reserve`. Claims remain unsettled while attaching and until a successful snapshot that began at least `settlement_seconds` after the task started, so a checkout cannot be erased by a snapshot that raced its launch. Stage-only solver features require a runner stage heartbeat and must not be declared persistent.
- `storage_guard_min_free_gb`: hold new task attaches and AEDT session starts on an account whose effective storage headroom drops below this, and record a `storage_guard` event, instead of letting work start into disk-quota-exceeded cascades. Effective headroom uses GPFS quota when available (otherwise `storage_quota_gb` minus measured usage) and subtracts the AEDT shadow reservation below. `0` disables storage admission.
- `aedt_storage_reservation_per_project_gb`: conservative future disk growth reserved per claimed AEDT session slot, attaching pooled project, or young running pooled project. The default is `4.0`; `0` disables only shadow reservation while retaining the observed-free guard. A three-project session therefore reserves 12 GB before its host launches.
- `aedt_storage_reservation_maturity_seconds`: keep a running pooled project's shadow reservation until it is this old relative to the quota observation that is being used (default `900`). Basing maturity on the observation time prevents a stale cached probe from releasing reservations before a fresh quota sample can reflect the writes. Terminal tasks release immediately; accounts are counted independently.
